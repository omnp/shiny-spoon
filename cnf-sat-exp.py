...
random.seed(1)
#import php
#clauses = php.php(6,5)#php.php(6,5)
#clauses = to3(clauses)
clauses3 = generate_assignment(4)
clauses3 = generate_full(clauses3,k=4,full=False)
clauses3 = to3(clauses3)
clauses3 = randomize_names(clauses3)
    ...
    for x in clauses3:
        ...
        for e in x:
            if e < 0:
                ...
                x_.add(vs_[abs(e)])
        x_ = tuple(sorted(x_,key=abs))
        clauses3_.add(x_)
    #print(clauses3_)
    #print('\t', wrapper(clauses3_))
    t = sat(clauses3_)
    print('\t', t)
    #clauses3 = randomize(clauses3)
"""
"""
random.seed(1)
#clauses = generate_assignment(n=6)
#clauses = set(partials(set.union(*(set(x) for x in generate_assignment(4))),full=False))
# # # # clauses = set(partials(set.union(*(set(x) for x in generate_assignment(4))),full=True))
#clauses = randomize(clauses)
clauses = generate_assignment(5)
clauses = generate_full(clauses,k=4,full=True)
#variables = set.union(*({abs(e) for e in x} for x in clauses))
#print(len(clauses), len(variables))
#print(sat(clauses))
##clauses = generate_full_alt(set.union(*(set(x) for x in generate_assignment(4))),k=4)
#clauses = generate_full_alt(set.union(*(set(x) for x in generate_assignment(9))),k=4)
clauses = to3(clauses)
variables = set.union(*({abs(e) for e in x} for x in clauses))
print(len(clauses), len(variables))
MaxAll = 0
clauses_ = set(clauses)
for i in range(1024):
    random.seed(i)
    clauses = set(clauses_)
    clauses = randomize(clauses)
    t = wrapper(clauses)#wrapper(clauses)#sat(clauses)
    print('\r', i, t)
    #if not t:
    #    print(clauses)
    #    break
