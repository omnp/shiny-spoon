...
There is also now an alternative procedure which breaks symmetries found in the CNF, such as in a clause the elements of which are interchangeable in which case only one of those elements needs to be tried, or in a pair of literals that are interchangable with additional clauses added based on that mapping.
